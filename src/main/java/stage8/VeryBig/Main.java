...
class Print implements Regex {
    ...
    public void print() {
        String statement;
        Map<String, BigInteger> variablesMap = new TreeMap<>();
        ...
        do {
            ...
            if (statement.contains("=")) {
                ...
                if (expression.checkExpression(newExpression, variablesMap)) {
                    ...
                    BigInteger result = converter.calculateExpression(postfix, variablesMap);
...
class Assignment implements Regex {
    public Boolean checkAssignment(String statement, Map<String, BigInteger> variablesMap) {
...
class Variables {
    public void variablesMap(String statement, Map<String, BigInteger> variablesMap) {
        String[] array = statement.split("=");
        String variable = array[0];
        String value = array[1];
        ...
        if (variablesMap.containsKey(variable) && variablesMap.containsKey(value)) {
            ...
            variablesMap.replace(variable, new BigInteger(value));
        }
        else if (variablesMap.containsKey(value)) {
            variablesMap.put(variable, variablesMap.get(value));
        }
        else {
            variablesMap.put(variable, new BigInteger(value));
        }
    }

    public void printVariableValue(String statement, Map<String, BigInteger> variablesMap) {
...
class Expression extends Converter implements Regex {
    ...
    public Boolean checkExpression(String expression, Map<String, BigInteger> variablesMap) {
...
class Converter implements Regex {
    ...
    public BigInteger calculateExpression(String postfix, Map<String, BigInteger> variablesMap) {
        String[] array = postfix.split(" ");
        BigInteger result = new BigInteger("0");
        BigInteger digit;
        BigInteger digit1, digit2;
        Stack<BigInteger> stack = new Stack<>();

        for (String next : array) {
            if (next.matches(Regex.digit)) {
                digit = new BigInteger(next);
                stack.push(digit);
            }
                ...
                switch (next) {
                    ...
                    case "+":
                        ...
                        break;
                    case "*":
                        result = digit1.multiply(digit2);
                        break;
                    case "/":
                        result = digit1.divide(digit2);
                        break;
                    default:
                        break;
                }
